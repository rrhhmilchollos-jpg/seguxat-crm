/**
 * seed_maria.cjs — Asignar las mismas 30 tareas de Karla a Maria (msolassanchis@gmail.com)
 * USO: node seed_maria.cjs
 */
...
async function main() {
  console.log('[seed_maria] Conectando...');
  await mongoose.connect(process.env.MONGODB_URI);
  console.log('[seed_maria] Conectado a:', mongoose.connection.name);

  const director = await Employee.findOne({ role: 'director' });
  if (!director) { console.error('❌ No se encontró al director.'); process.exit(1); }

  const maria = await Employee.findOne({ email: 'msolassanchis@gmail.com' });
  if (!maria) {
    console.error('❌ No se encontró el empleado con email msolassanchis@gmail.com');
    console.error('   Comprueba que el email está registrado en el CRM.');
    await mongoose.disconnect(); process.exit(1);
  }
  console.log(`[seed_maria] ✅ Empleado encontrado: ${maria.name} (${maria.role} · ${maria.zone})`);

  let añadidas = 0;
  let duplicadas = 0;

  for (const t of TAREAS) {
    const yaExiste = maria.tasks.some(x => x.title === t.title);
    if (!yaExiste) {
      maria.tasks.push({ ...t, done: false, createdBy: director._id });
      añadidas++;
    } else {
      duplicadas++;
    }
  ...
  console.log(`\n[seed_maria] ✅ ${añadidas} tareas nuevas añadidas a ${maria.name}`);
  if (duplicadas > 0) console.log(`[seed_maria] ℹ️  ${duplicadas} tareas ya existían (omitidas)`);
  console.log(`[seed_maria] 📋 Total tareas en CRM: ${maria.tasks.length}`);
  console.log('[seed_maria] ✅ Listo!');

  await mongoose.disconnect();
  process.exit(0);
}

main().catch(e => { console.error('[seed_maria] ❌ Error:', e.message); process.exit(1); });
